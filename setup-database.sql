...
CREATE POLICY "Users can view own bookings" ON bookings FOR SELECT USING (auth.email() = client_email);
...
CREATE TRIGGER update_logo_settings_updated_at BEFORE UPDATE ON logo_settings FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
-- Success message
SELECT 'SOULPATH Database setup completed successfully!' as message;
